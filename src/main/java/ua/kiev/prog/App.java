    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for(Course course:courses){
                query = em.createNativeQuery("SELECT count(course_id) FROM my_db.clientcourse where course_id = ?;");
                query.setParameter(1,course.getId());
                BigInteger bigInt = (BigInteger) query.getSingleResult();
                System.out.println("На курсе " + course.getName() + " учатся " + bigInt + " клиентов.");
            }
        } finally {
            em.close();
            emf.close();
